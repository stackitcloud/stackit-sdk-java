...
public class UpdateLoadBalancerPayload {
	...
	/**
	 * The external IP address where this Application Load Balancer is exposed. Not changeable after
	 * creation.
	 *
	...
	/**
	 * Service Plan configures the size of the Application Load Balancer. Currently supported plan
	 * is p10. This list can change in the future where plan ids will be removed and new plans by
	 * added. That is the reason this is not an enum.
	 *
	 * @return planId
	 */
	...
	/**
	 * Application Load Balancer resource version. Must be empty or unset for creating Application
	 * Load Balancers, non-empty for updating. Semantics: While retrieving, this is the current
	 * version of this Application Load Balancer resource that changes during updates. On updates
	 * this field specified the Application Load Balancer version you calculated your update for
	 * instead of the future version to enable concurrency safe updates. Update calls will then
	 * report the new version in their result as you would see with a Application Load Balancer
	 * retrieval call later. There exist no total order of the version, so you can only compare it
	 * for equality, but not for less/greater than another version. Since the creation of
	 * Application Load Balancer is always intended to create the first version of it, there should
	 * be no existing version. That&#39;s why this field must by empty of not present in that case.
	 *
	 * @return version
	 */
